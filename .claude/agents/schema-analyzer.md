...
The `schema/` directory contains large JSON schema files (`infra.json`, `manage.json`) that define the expected API structure. This agent compares those schemas against the actual endpoint implementations in `app/v1/` to find:
...
2. Scan `app/v1/endpoints/` to catalog all implemented routes
3. Scan `app/v1/models/` to catalog all defined models and their fields
