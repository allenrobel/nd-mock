...
from .....models.switch import ListSwitchesMetadata, ListSwitchesResponseBody, SwitchDataResponse, SwitchDbModelV2
...
def v2_switches_get(
    *,
    ...
    total = session.exec(select(func.count()).select_from(SwitchDbModelV2).where(SwitchDbModelV2.fabricName == fabric_name)).one()
    db_switches = session.exec(select(SwitchDbModelV2).where(SwitchDbModelV2.fabricName == fabric_name).offset(offset).limit(limit)).all()
